#!/usr/bin/env python3
"""Render the headless-client design to a shareable HTML page.
...
def render(md):
    ...
    while i < n:
        ...
        if m:
            ...
            if depth == 1:
                i += 1
                continue                                  # the title lives in the hero
            if depth == 2:
...
def main(argv):
    ...
    page = """<title>Sovereign headless client — target-state design</title>
<style>%s</style>
...
<main>
<header class="hero"><p class="eyebrow">OpenStarbound · target-state architecture · task #204</p><h1>Sovereign Decoupled Headless Client</h1><p class="standfirst">%s</p><div class="banner"><span class="chip proposed">work in progress</span><p>%s</p></div><dl class="meta"><div><dt>approved</dt><dd>none</dd></div><div><dt>provisional</dt><dd>Sections 1, 4, 5</dd></div><div><dt>components</dt><dd>%d in 4 zone directories</dd></div><div><dt>runtime elements</dt><dd>%d</dd></div><div><dt>grant rows</dt><dd>%d</dd></div><div><dt>snapshot of</dt><dd><code>%s</code> on <code>%s</code></dd></div></dl><p class="note">A rendered copy of <code>docs/superpowers/specs/2026-08-01-sovereign-headless-client-design.md</code>, generated by <code>scripts/spec-artifact.py</code>. The repository holds the authoritative version; this page does not update with it.</p></header>
%s
</main>
</div>
""" % (css, "".join(nav), goal_txt, banner_txt, ncomp, nelem, ngrants, sha, branch, body)

    pathlib.Path(args.out).write_text(page, encoding="utf-8")
    print("spec-artifact: wrote %s (%.1f KB, %d sections)"
          % (args.out, len(page) / 1024, len(nav)))
    return 0
