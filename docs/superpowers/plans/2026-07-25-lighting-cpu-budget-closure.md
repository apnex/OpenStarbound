...
(cd dist && ./game_tests --gtest_filter='LightingTelemetry.*')
```
...
(cd dist && ./core_tests --gtest_filter='Telemetry.*')
```
...
scripts/render-gate.sh && (cd dist && ./core_tests) && (cd dist && ./game_tests)
```
