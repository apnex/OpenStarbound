...
**161 tasks** across 2 store(s): 3 in_progress, 35 pending, 123 completed
...
| [#221](#c29c1332-221) | `c29c1332` | open | GATHER-2 (hardening-framed, reordered): E02 observability -&gt; E01 testability -&gt; E03 dedupe -&gt; E04 correctness | — | — |
...
#### #221 — GATHER-2 (hardening-framed, reordered): E02 observability -&gt; E01 testability -&gt; E03 dedupe -&gt; E04 correctness
...
THE PERF CASE, MEASURED AND SET ASIDE. With GPU lighting on (the shipped default), across the harness profiles:
    lighting CPU     2.3 - 8.2% of active frame CPU
    the whole gather 0.6 - 3.2%     (01-Lava Refinery 3.20%, 00-Ocean-Lab 3.03%, 04-Ocean Factory 1.17%, Desert Town 0.59%)
and the client idles 60-77% of every frame (of-true.json: 12,464 us idle of 16,193 us). E03's ceiling -- the zero-fill it deletes -- is 393-573 KB (24,576-35,840 GatherCells x 16 B) on the scroll path only: order 0.3% of active frame CPU, below the live profile's A/B resolution. Expect MEASURED NULL, and do not ship any of these claiming a perf win. (With GPU lighting OFF, lighting CPU is 20-52% -- that is a different problem and not this ticket's.)
CAVEAT on the denominator: "active CPU" = cpu.frame.total.us - cpu.frame.idle.us. meta.vsync=False yet frames pace at ~16 ms, so something limits them. If that is a frame limiter the idle is genuine headroom and the shares above hold; if it is a hidden stall they understate. Not established.

ORDER CHANGED, and why. It was E02, E03, E01, E04. E03 before E01 is backwards. Standalone, E03 DUPLICATES the rect derivation that currently lives inline in shiftAndGatherMargin: it trades one line with a trivially checkable invariant (everything starts zeroed) for two rect zeros that must stay in step with two gather rects. That is more coupling, not less. It becomes a real dedupe only once E01 has extracted that math into a single helper feeding both.

--- 1. E02 -- OBSERVABILITY. The instrument the other three are judged by.
Three counters (hit / scroll / full) plus a reason tag on the full path (epoch, dims, anchor jump, first frame), at the existing A1/A2 cache branch in lightingCalc. The seam is already open: #210 added lighting.gather.calc_outside_loaded in the same block.
WHY FIRST: we shipped A1/A2 on the premise that scrolling is common and have NEVER measured the hit/scroll/full split. E03 only helps the scroll path; E01 only matters if that path is live; E04's oracle can pass while testing nothing if it only ever drives the full-gather path -- the unarmed-oracle false green this project keeps hitting. Make the counters SEVERITY-bearing, not boolean: calc_outside_loaded reads 0 or 100%-of-recomputes and nothing between, so it cannot say how bad, only whether. Same trap as lighting.gpu.spread.passes saturating at its cap (#224).

--- 2. E01 -- TESTABILITY. The least-tested code in the lighting path.
Extract the A2 scroll math (delta -> overlap copy bounds + the two margin rects) from shiftAndGatherMargin into a pure, header-only helper, and pin it with a differential unit test over synthetic deltas. Feasible now because #214 left one gather rather than two forked ones.
WHY: this is the most off-by-one-prone arithmetic in the cluster -- four sign cases, half-open bounds, an intra-column slice copy -- and it has no unit test at all. It is also the precondition that makes E03 a dedupe instead of a duplication.
...
Verify with the render gate's lightingGatherCache A/B, which as of cb17d323 actually judges its result, and which as of 6e66f36e carries a null control.
